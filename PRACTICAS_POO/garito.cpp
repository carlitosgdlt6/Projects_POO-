...
Garito::Garito(std::string nombre, std::string direccion) :
    ...
    _direccion(direccion) {
        if ( nombre.length() < 3 )
            throw ParametroNoValido( "[Garito::Garito]","El nombre debe tener al menos 3 caracteres" );
        if ( direccion == "" )
            throw ParametroNoValido ( "[Garito::Garito]"," Se intenta asignar una dirección vacía" );
}
...
void Garito::setDireccion(std::string direccion) {
   if ( direccion == "" )
      throw ParametroNoValido ( "Garito::setDireccion","Se intenta asignar una dirección vacía" );
...
void Garito::setNombre(std::string nombre) {
   if ( nombre.length() < 3 )
      throw ParametroNoValido ( "[Garito::setNombre]"," El nombre debe tener al menos 3 caracteres" );
